...
#!/bin/bash
# Continuous ablation runs with different window scales for RGBNT100 dataset.
# Usage: bash run_rgbnt100_baseline_continuous.sh
#
# 功能：
# 1. 一直循环，每轮循环之间休息5分钟
...
#    - [4,16] (4x4+16x16组合)
#    - [8,16] (8x8+16x16组合)
# 3. 使用数据集：RGBNT100（训练集、验证集、测试集）
# 4. 使用配置文件：configs/RGBNT100/jzb_baseline_optimize.yml
...
CFG="configs/RGBNT100/jzb_baseline_optimize.yml"
BASE_OUT="./outputs/rgbnt100_multiscale"

# 显示数据集信息
echo "========================================"
echo "📊 RGBNT100 数据集信息"
echo "========================================"
echo "  训练集: 7815 张图像（50 个车辆）"
echo "  验证集: 860 张图像（用于训练监控）"
echo "  测试集: 10290 张图像（1715 查询 + 8575 图库）"
echo "========================================"
echo ""
...
while true; do
  ...
  for i in "${!SCALE_LIST[@]}"; do
    ...
    echo "  输出目录: ${OUT_DIR}"
    echo "  训练集: 7815 张图像"
    echo "  验证集: 860 张图像（用于监控）"
    echo "  测试集: 10290 张图像（最终评估）"
    echo "========================================"
    echo ""
