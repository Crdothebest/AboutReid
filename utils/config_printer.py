...
def print_final_config(cfg):
    ...
    if cfg.MODEL.USE_MULTI_SCALE_MOE:
        ...
        print(f"    * 专家隐藏层维度 ExpertHiddenDim = {cfg.MODEL.MOE_EXPERT_HIDDEN_DIM}")
        print(f"    * 门控网络温度 GatingTemperature = {cfg.MODEL.MOE_TEMPERATURE}")
    ...
    if cfg.MODEL.USE_MULTI_SCALE_MOE:
        ...
        print(f"  - MoE平衡损失权重 MoEBalanceWeight = {balance_weight}")
        print(f"  - MoE多样性损失权重 MoEDiversityWeight = {diversity_weight}")
        print(f"  - MoE稀疏性损失权重 MoESparsityWeight = {sparsity_weight}")
